...
async function uploadToCloudinary(buffer, folder = "foryqueen") {
  try {
...
async function handleProductCreate(req, res) {
  ...
  for (const file of req.files || []) {
    const result = await uploadToCloudinary(file.buffer, "foryqueen/products");
    if (result) {
      images.push({
        url: result.url,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (pathname === "/api/upload" && method === "POST") {
      ...
      const result = await uploadToCloudinary(file.buffer, "foryqueen/uploads");
